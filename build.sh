...
CLASS_SIZE=$(printf '#include "include/vulcan/lock_free_spsc_queue.hpp"\n#include <iostream>\nint main(){std::cout << sizeof(LockFreeSPSCQueue<QueueOrder, 256>);}' | g++ -x c++ -I./include -o size_check - && ./size_check && rm size_check)
echo "The size is: $CLASS_SIZE"

#memory calculation logic:
CALCULATED_REQUIREMENT=$(((CLASS_SIZE + 2097151) / 2097152))
