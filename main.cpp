...
void push_orders_to_queue(LockFreeSPSCQueue<QueueOrder, 256>& queue, std::atomic<bool>& m_start_ref, int core_id) {
    ...
    while (!m_start_ref.load(std::memory_order_acquire)) {
        _mm_pause(); // notify LSU that the thread is in a spin-wait, reducing power consumption and preventing pipeline from being corrupted with speculative loads
    }
...
void pop_orders_from_queue(LockFreeSPSCQueue<QueueOrder, 256>& queue, std::atomic<bool>& m_start_ref, int core_id) {
    ...
    for (size_t i = 0; i < TOTAL_BURST_ORDERS; i+=8) {
        ...
        const QueueOrder* current_head_index_ptr_0 = queue.consumer_uncommitted_peek(local_head_idx);
        const QueueOrder* current_head_index_ptr_1 = queue.consumer_uncommitted_peek(local_head_idx + 1);
        const QueueOrder* current_head_index_ptr_2 = queue.consumer_uncommitted_peek(local_head_idx + 2);
        const QueueOrder* current_head_index_ptr_3 = queue.consumer_uncommitted_peek(local_head_idx + 3);
        const QueueOrder* current_head_index_ptr_4 = queue.consumer_uncommitted_peek(local_head_idx + 4);
        const QueueOrder* current_head_index_ptr_5 = queue.consumer_uncommitted_peek(local_head_idx + 5);
        const QueueOrder* current_head_index_ptr_6 = queue.consumer_uncommitted_peek(local_head_idx + 6);
        const QueueOrder* current_head_index_ptr_7 = queue.consumer_uncommitted_peek(local_head_idx + 7);
