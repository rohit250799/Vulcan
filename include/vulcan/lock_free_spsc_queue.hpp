...
public:
  ...
  void producer_uncommitted_push(QueueOrder &qOrder,
                                 size_t current_local_tail) noexcept;
  const QueueOrder *
  consumer_uncommitted_peek(size_t consumer_local_head) noexcept;
  ...
  void destroy_mapping() {
    size_t size_required = sizeof(LockFreeSPSCQueue<T, capacity>);
    this->~LockFreeSPSCQueue<T, capacity>();
    munmap(this, size_required);
  }
...
template <typename T, size_t capacity>
void LockFreeSPSCQueue<T, capacity>::producer_uncommitted_push(
    QueueOrder &qOrder, size_t current_local_tail) noexcept {
  char *buffer_base = reinterpret_cast<char *>(this) + 128;
  size_t slot_index = current_local_tail & (capacity - 1);
  size_t byte_offset = slot_index << 6;
  char *slot_address = buffer_base + byte_offset;
  QueueOrder *casted_buffer_address =
      reinterpret_cast<QueueOrder *>(slot_address);
  new (casted_buffer_address) QueueOrder(qOrder);
  return;
}

template <typename T, size_t capacity>
const QueueOrder *LockFreeSPSCQueue<T, capacity>::consumer_uncommitted_peek(
    size_t consumer_local_head) noexcept {
  ...
  const QueueOrder *slot =
      reinterpret_cast<const QueueOrder *>(buffer_base + byte_offset);
  return slot;
}

template <typename T, size_t capacity>
void LockFreeSPSCQueue<T, capacity>::publish_tail_release(
    size_t new_local_tail) {
  mProducer.tail.store(new_local_tail, std::memory_order_release);
  return;
}

template <typename T, size_t capacity>
void LockFreeSPSCQueue<T, capacity>::publish_head_release(
    size_t new_local_head) {
  mConsumer.head.store(new_local_head, std::memory_order_release);
  return;
}

template <typename T, size_t capacity>
[[nodiscard]] __restrict const T *
LockFreeSPSCQueue<T, capacity>::buffer() noexcept {
  return reinterpret_cast<T *>(reinterpret_cast<char *>(this) + 2 * 64);
}

template <typename T, size_t capacity>
LockFreeSPSCQueue<T, capacity>::LockFreeSPSCQueue()
    : mProducer{0, 0}, mConsumer{0, 0} {
  assert(sizeof(Metadata_Producer) == 64 &&
         "Size of metadata producer should be 64 bytes \n");
  assert(sizeof(Metadata_Consumer) == 64 &&
         "Size of metadata consumer should be 64 bytes \n");
  assert(reinterpret_cast<uintptr_t>(this) % 64 == 0 &&
         "Assertion failed: Non functional core.. \n");
  assert(mProducer.mask == mConsumer.mask &&
         "Mask in both producer and consumer threads should be equal \n");
  assert(sizeof(QueueOrder) % 64 == 0 &&
         "QueueOrders should be 64 bytes in size \n");
}
...
LockFreeSPSCQueue<T, capacity> *LockFreeSPSCQueue<T, capacity>::create() {
  ...
  assert(current_huge_pages == 1 &&
         "Assertion failed: Current huge pages value is not 1, allocation "
         "unsuccessful \n");
  ...
  auto *mem_ptr =
      mmap(NULL, rounded_size, PROT_READ | PROT_WRITE,
  ...
  int mbind_policy_result =
      mbind(mem_ptr, boundary, MPOL_BIND, &nodemask, maxnode, MPOL_MF_STRICT);
  ...
  assert(((uintptr_t)offset_ptr % L1_stride) != 0 &&
         "Buffer offset should not be a multiple of L1 critical stride \n");
...
template <typename T, size_t capacity>
inline void
LockFreeSPSCQueue<T, capacity>::commit_pop_order_from_queue(const T *ptr,
                                                            size_t index) {
  constexpr bool is_non_pod =
