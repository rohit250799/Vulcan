...
bool populate_stack_buffer(std::array<QueueOrder, 8>& my_queueOrder) {
    ...
    assert(my_queueOrder[7].order_id == 7 && "Assertion failed: Stack buffer not populated correctly \n");
    return true;
}

void warm_up_producer(LockFreeSPSCQueue<QueueOrder, 256>& lock_free_queue_ref, std::array<QueueOrder, 8>& queue_orders_ref) {
    ...
    for (size_t i = 0; i < TOTAL_WARMUP_OPS; ++i) {
        ...
        lock_free_queue_ref.producer_uncommitted_push(queue_orders_ref[buffer_index], local_current_tail);
